...
var SftpFeature = (function () {
    function SftpFeature() {
    }
    SftpFeature.HARDLINK = "LINK";
    SftpFeature.POSIX_RENAME = "POSIX_RENAME";
    return SftpFeature;
})();
var SftpClientCore = (function () {
    ...
    SftpClientCore.prototype._init = function (host, log, callback) {
        var _this = this;
        if (this._host)
        ...
        this._debug = level <= 20 /* DEBUG */;
        this._trace = level <= 10 /* TRACE */;
        ...
        this.execute(request, callback, function (response, cb) {
            ...
            while ((response.length - response.position) >= 4) {
                ...
                if (extensionName.indexOf("@openssh.com") === (extensionName.length - 12)) {
                    ...
                    var val = _this._extensions[extensionName];
                ...
                _this._extensions[extensionName] = value;
            }
            _this._log.debug(_this._extensions, "[%d] - Server extensions", _this._sessionId);
            if (SftpExtensions.contains(_this._extensions[SftpExtensions.HARDLINK], "1")) {
                _this._features[SftpFeature.HARDLINK] = SftpExtensions.HARDLINK;
            }
            if (SftpExtensions.contains(_this._extensions[SftpExtensions.POSIX_RENAME], "1")) {
                _this._features[SftpFeature.POSIX_RENAME] = SftpExtensions.POSIX_RENAME;
            }
            _this._ready = true;
            callback(null);
        }, info);
    ...
    SftpClientCore.prototype.unlink = function (path, callback) {
        this.checkCallback(callback);
        path = this.checkPath(path, 'path');
        this.command(13 /* REMOVE */, [path], callback, this.parseStatus, { command: "unlink", path: path });
    };
    ...
    SftpClientCore.prototype.rename = function (oldPath, newPath, flags, callback) {
        var _this = this;
        this.checkCallback(callback);
        ...
        switch (flags) {
            ...
            case 0:
                ...
                break;
            default:
                process.nextTick(function () { return callback(_this.createError(8 /* OP_UNSUPPORTED */, "Unsupported rename flags", info)); });
                break;
        }
    ...
    SftpClientCore.prototype.checkCallback = function (callback) {
    ...
    SftpClientCore.prototype.command = function (command, args, callback, responseParser, info) {
        ...
        if (!command) {
            process.nextTick(function () { return callback(_this.createError(8 /* OP_UNSUPPORTED */, "Operation not supported", info)); });
            return;
        }
    ...
    SftpClientCore._nextSessionId = 1;
...
var SftpClient = (function (_super) {
    ...
    SftpClient.prototype.bind = function (channel, options, callback) {
        var _this = this;
        if (typeof callback === "undefined" && typeof options === "function") {
            callback = options;
            options = null;
        }
        return _super.prototype._task.call(this, callback, function (callback) { return _this._bind(channel, options, callback); });
    };
    SftpClient.prototype._bind = function (channel, options, callback) {
        var _this = this;
        var sftp = this._fs;
        if (this._bound)
            throw new Error("Already bound");
        ...
        sftp._init(channel, log, function (err) {
            if (err) {
                sftp.end();
                _this._bound = false;
                callback(err);
