...
var SftpServerSession = (function () {
    function SftpServerSession(channel, fs, emitter, log, meta) {
        var _this = this;
        ...
        channel.on("message", function (packet) {
            try {
                ...
                log.error({ "err": err }, "[%d] - Error while accepting request", _this._id);
        ...
        channel.on("close", function (err) {
            if (!err) {
                log.info("[%d] - Session closed by the client", _this._id);
            }
                ...
                log.error({ "err": err }, "[%d] - Session failed", _this._id);
            }
            _this.end();
            if (!emitter.emit("closedSession", _this, err)) {
                if (err) {
        ...
        try {
            ...
            switch (request.type) {
                ...
                case 5 /* READ */:
                    ...
                    fs.read(handleInfo.handle, response.buffer, offset, count, position, function (err, b, bytesRead) {
                        if (_this.sendIfError(response, err))
                            return;
                        if (bytesRead == 0) {
                ...
                case 18 /* RENAME */:
                    ...
                    fs.rename(oldpath, newpath, 0, function (err) { return _this.sendSuccess(response, err); });
                    return;
                case 19 /* READLINK */:
                    var path = request.readString();
                ...
                case SftpExtensions.POSIX_RENAME:
                    var oldpath = request.readString();
                    var newpath = request.readString();
                    fs.rename(oldpath, newpath, 1 /* OVERWRITE */, function (err) { return _this.sendSuccess(response, err); });
                    return;
                default:
